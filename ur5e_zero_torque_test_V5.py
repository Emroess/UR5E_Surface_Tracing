...
def build_urscript(
  ...
  # Stream telemetry back to the Python terminal. This avoids using textmsg()
  # for high-rate samples and keeps PolyScope's log focused on major events.
  def send_telemetry(label, value):
    socket_send_string(label, "telemetry")
    socket_send_line(to_str(value), "telemetry")
  end

  def zero_torque_test(duration_s, max_torque_nm, max_torque_rate_nm_s, max_tcp_force_n, max_tcp_torque_nm, sample_period_s, log_torque_loop_state, joint_damping_test, joint_damping):
    if joint_damping_test:
    ...
    if log_torque_loop_state:
      ...
      if telemetry_open:
        ...
        textmsg("Telemetry socket open failed; continuing without terminal telemetry")
      end
    end
    ...
    while elapsed < duration_s:
      ...
      if safety_ok:
        if joint_damping_test:
          ...
          tau_cmd = [-joint_damping[0] * joint_speeds[0], -joint_damping[1] * joint_speeds[1], -joint_damping[2] * joint_speeds[2], -joint_damping[3] * joint_speeds[3], -joint_damping[4] * joint_speeds[4], -joint_damping[5] * joint_speeds[5]]
        else:
  ...
  zero_torque_test({duration_s:.6f}, {max_torque_nm:.6f}, {max_torque_rate_nm_s:.6f}, {max_tcp_force_n:.6f}, {max_tcp_torque_nm:.6f}, {sample_period_s:.6f}, {log_state}, {damping_test}, {joint_damping_values})
end
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            "Six joint damping gains in Nms/rad for --joint-damping-test. "
...
def main():
    ...
    if args.joint_damping_test and args.use_friction_scales:
        print(
            "ERROR: --joint-damping-test uses the tested friction_comp=True baseline; "
    ...
    try:
        if telemetry_enabled:
            with make_telemetry_server(args.telemetry_bind_ip, args.telemetry_port) as server:
                ...
                telemetry_status = receive_telemetry(server)
    ...
    if args.smoke_test:
        ...
        print("EE pose, speed, and wrench samples were streamed to this terminal.")
    else:
        if args.joint_damping_test:
        ...
        if args.log_torque_loop_state:
            print("In-loop q, qd, EE state, wrench, and tau_safe samples were streamed to this terminal.")

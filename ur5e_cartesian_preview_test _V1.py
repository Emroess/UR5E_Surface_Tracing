#!/usr/bin/env python3
#
# UR5e Cartesian impedance preview commissioning test.
#
# SAFETY WARNINGS:
# - Run only in free space.
# - Confirm payload and TCP are correct before running.
# - Keep the E-stop accessible.
# - Cartesian preview telemetry computes a virtual EE wrench for inspection
#   only; it does not map that wrench into joint torques yet.
# - Zero torque mode means zero additional commanded torque; UR internally
...
DEFAULT_CARTESIAN_STIFFNESS = [50.0, 50.0, 50.0, 0.0, 0.0, 0.0]
DEFAULT_CARTESIAN_DAMPING = [20.0, 20.0, 20.0, 0.0, 0.0, 0.0]
...
def expand_joint_values(values, option_name):
    """Accept one value for all joints or six values in UR joint order."""
    ...
    raise ValueError(
        f"{option_name} expects either 1 value or {NUM_JOINTS} values "
        f"in this order: {JOINT_ORDER}."
    )


def expand_cartesian_values(values, option_name):
    """Accept one value for all EE axes or six values in pose-vector order."""
    if isinstance(values, (int, float)):
        values = [values]
    values = list(values)
    ...
    raise ValueError(
        f"{option_name} expects either 1 value or 6 values "
        f"in this order: {CARTESIAN_AXIS_ORDER}."
    )
...
def build_urscript(
    ...
    cartesian_preview_telemetry=False,
    cartesian_stiffness=DEFAULT_CARTESIAN_STIFFNESS,
    cartesian_damping=DEFAULT_CARTESIAN_DAMPING,
    telemetry_host="",
    ...
    cartesian_preview = "True" if cartesian_preview_telemetry else "False"
    max_torque_values = format_urscript_list(
        expand_joint_values(max_torque_nm, "max_torque_nm")
    )
    max_torque_rate_values = format_urscript_list(
        expand_joint_values(max_torque_rate_nm_s, "max_torque_rate_nm_s")
    )
    joint_damping_values = format_urscript_list(joint_damping)
    joint_stiffness_values = format_urscript_list(joint_stiffness)
    cartesian_stiffness_values = format_urscript_list(cartesian_stiffness)
    cartesian_damping_values = format_urscript_list(cartesian_damping)
    telemetry_host_literal = format_urscript_string(telemetry_host)

    return f"""def ur5e_cartesian_preview_socket_program():
  ...
  # Limit commanded joint torque independently per joint. Future controllers
  # should replace tau_cmd, not bypass this clamp.
  def clamp_torque(tau_cmd, max_torque_nm):
    return [clamp_value(tau_cmd[0], max_torque_nm[0]), clamp_value(tau_cmd[1], max_torque_nm[1]), clamp_value(tau_cmd[2], max_torque_nm[2]), clamp_value(tau_cmd[3], max_torque_nm[3]), clamp_value(tau_cmd[4], max_torque_nm[4]), clamp_value(tau_cmd[5], max_torque_nm[5])]
  end

  # Limit how quickly torque can change independently per joint. This lets the
  # larger axes use stronger spring torques without making wrist axes snappy.
  def rate_limit_torque(tau_cmd, previous_tau, max_torque_rate_nm_s, dt):
    max_delta = [max_torque_rate_nm_s[0] * dt, max_torque_rate_nm_s[1] * dt, max_torque_rate_nm_s[2] * dt, max_torque_rate_nm_s[3] * dt, max_torque_rate_nm_s[4] * dt, max_torque_rate_nm_s[5] * dt]
    return [previous_tau[0] + clamp_value(tau_cmd[0] - previous_tau[0], max_delta[0]), previous_tau[1] + clamp_value(tau_cmd[1] - previous_tau[1], max_delta[1]), previous_tau[2] + clamp_value(tau_cmd[2] - previous_tau[2], max_delta[2]), previous_tau[3] + clamp_value(tau_cmd[3] - previous_tau[3], max_delta[3]), previous_tau[4] + clamp_value(tau_cmd[4] - previous_tau[4], max_delta[4]), previous_tau[5] + clamp_value(tau_cmd[5] - previous_tau[5], max_delta[5])]
  end
  ...
  # Cartesian preview math uses component-wise pose-vector subtraction. This is
  # enough for small free-space sign checks; later Cartesian torque control
  # should replace the rotational part with a proper orientation error.
  def pose_error_vector(target_pose, actual_pose):
    return [target_pose[0] - actual_pose[0], target_pose[1] - actual_pose[1], target_pose[2] - actual_pose[2], target_pose[3] - actual_pose[3], target_pose[4] - actual_pose[4], target_pose[5] - actual_pose[5]]
  end

  def cartesian_spring_wrench(cartesian_error, cartesian_stiffness):
    return [cartesian_stiffness[0] * cartesian_error[0], cartesian_stiffness[1] * cartesian_error[1], cartesian_stiffness[2] * cartesian_error[2], cartesian_stiffness[3] * cartesian_error[3], cartesian_stiffness[4] * cartesian_error[4], cartesian_stiffness[5] * cartesian_error[5]]
  end

  def cartesian_damping_wrench(tcp_speed, cartesian_damping):
    return [-cartesian_damping[0] * tcp_speed[0], -cartesian_damping[1] * tcp_speed[1], -cartesian_damping[2] * tcp_speed[2], -cartesian_damping[3] * tcp_speed[3], -cartesian_damping[4] * tcp_speed[4], -cartesian_damping[5] * tcp_speed[5]]
  end

  def add_six(left, right):
  ...
  def zero_torque_test(duration_s, max_torque_nm, max_torque_rate_nm_s, max_tcp_force_n, max_tcp_torque_nm, sample_period_s, log_torque_loop_state, joint_damping_test, joint_impedance_test, joint_damping, joint_stiffness, cartesian_preview_telemetry, cartesian_stiffness, cartesian_damping):
    ...
    if cartesian_preview_telemetry:
      textmsg("Cartesian preview telemetry enabled; preview wrench is not commanded")
    end
    ...
    if log_torque_loop_state:
      ...
      if telemetry_open:
        ...
        if cartesian_preview_telemetry:
          send_telemetry("x_hold [x,y,z,rx,ry,rz]=", x_hold)
          send_telemetry("cartesian_stiffness=", cartesian_stiffness)
          send_telemetry("cartesian_damping=", cartesian_damping)
        end
      else:
    ...
    while elapsed < duration_s:
      ...
      cartesian_spring = [0, 0, 0, 0, 0, 0]
      cartesian_damping_term = [0, 0, 0, 0, 0, 0]
      cartesian_wrench_cmd = [0, 0, 0, 0, 0, 0]

      if cartesian_preview_telemetry:
        cartesian_error = pose_error_vector(x_hold, tcp_pose)
        cartesian_spring = cartesian_spring_wrench(cartesian_error, cartesian_stiffness)
        cartesian_damping_term = cartesian_damping_wrench(tcp_speed, cartesian_damping)
        cartesian_wrench_cmd = add_six(cartesian_spring, cartesian_damping_term)
      end
      ...
      if safety_ok:
        ...
        if log_torque_loop_state and telemetry_open and elapsed >= next_sample:
          ...
          if cartesian_preview_telemetry:
            send_telemetry("cartesian_error [x,y,z,rx,ry,rz]=", cartesian_error)
            send_telemetry("cartesian_spring_wrench [fx,fy,fz,tx,ty,tz]=", cartesian_spring)
            send_telemetry("cartesian_damping_wrench [fx,fy,fz,tx,ty,tz]=", cartesian_damping_term)
            send_telemetry("cartesian_wrench_cmd [fx,fy,fz,tx,ty,tz]=", cartesian_wrench_cmd)
          end
  ...
  zero_torque_test({duration_s:.6f}, {max_torque_values}, {max_torque_rate_values}, {max_tcp_force_n:.6f}, {max_tcp_torque_nm:.6f}, {sample_period_s:.6f}, {log_state}, {damping_test}, {impedance_test}, {joint_damping_values}, {joint_stiffness_values}, {cartesian_preview}, {cartesian_stiffness_values}, {cartesian_damping_values})
end
"""
...
def create_telemetry_log(args, telemetry_host):
    ...
    with log_path.open("w", encoding="utf-8") as log_file:
        ...
        log_file.write(f"# cartesian_preview_telemetry={args.cartesian_preview_telemetry}\n")
        if args.cartesian_preview_telemetry:
            log_file.write(f"# cartesian_stiffness={args.cartesian_stiffness}\n")
            log_file.write(f"# cartesian_damping={args.cartesian_damping}\n")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        metavar="Nm",
        help=(
            "Maximum absolute commanded torque in Nm. Provide one value for "
            "all joints or six values in this order: "
            f"{JOINT_ORDER}. Default: {DEFAULT_MAX_TORQUE_NM}."
        ),
    )
    parser.add_argument(
        "--max-torque-rate",
        ...
        metavar="Nm_s",
        help=(
            "Maximum torque change rate in Nm/s. Provide one value for all "
            "joints or six values in this order: "
            f"{JOINT_ORDER}. Default: {DEFAULT_MAX_TORQUE_RATE_NM_S}."
        ),
    )
    parser.add_argument(
        "--max-tcp-force",
    ...
    parser.add_argument(
        "--cartesian-preview-telemetry",
        action="store_true",
        help=(
            "Inside the direct_torque loop, capture the starting TCP pose and "
            "log Cartesian pose error plus a virtual spring-damper wrench. "
            "The preview wrench is logged only; it is not commanded."
        ),
    )
    parser.add_argument(
        "--joint-damping-test",
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        default=DEFAULT_CARTESIAN_STIFFNESS,
        metavar="K",
        help=(
            "Cartesian preview stiffness gains. Provide one value for all EE "
            "axes or six values in this order: "
            f"{CARTESIAN_AXIS_ORDER}. Translation units are N/m; rotation "
            f"units are Nm/rad. Default: {DEFAULT_CARTESIAN_STIFFNESS}."
        ),
    )
    parser.add_argument(
        "--cartesian-damping",
        type=float,
        nargs="+",
        default=DEFAULT_CARTESIAN_DAMPING,
        metavar="D",
        help=(
            "Cartesian preview damping gains. Provide one value for all EE "
            "axes or six values in this order: "
            f"{CARTESIAN_AXIS_ORDER}. Translation units are N/(m/s); rotation "
            f"units are Nm/(rad/s). Default: {DEFAULT_CARTESIAN_DAMPING}."
        ),
    )
    parser.add_argument(
        "--use-friction-scales",
...
def main():
    ...
    try:
        args.max_torque = expand_joint_values(args.max_torque, "--max-torque")
        args.max_torque_rate = expand_joint_values(
            args.max_torque_rate, "--max-torque-rate"
        )
        args.cartesian_stiffness = expand_cartesian_values(
            args.cartesian_stiffness, "--cartesian-stiffness"
        )
        args.cartesian_damping = expand_cartesian_values(
            args.cartesian_damping, "--cartesian-damping"
        )
    except ValueError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2

    if any(limit <= 0 for limit in args.max_torque):
        print("ERROR: --max-torque values must be greater than 0.", file=sys.stderr)
        return 2

    if any(limit <= 0 for limit in args.max_torque_rate):
        print("ERROR: --max-torque-rate values must be greater than 0.", file=sys.stderr)
        return 2
    ...
    if any(gain < 0 for gain in args.cartesian_stiffness):
        print("ERROR: --cartesian-stiffness values must be zero or greater.", file=sys.stderr)
        return 2

    if any(gain < 0 for gain in args.cartesian_damping):
        print("ERROR: --cartesian-damping values must be zero or greater.", file=sys.stderr)
        return 2
    ...
    if args.cartesian_preview_telemetry and (args.smoke_test or args.ee_state_readout):
        print(
            "ERROR: --cartesian-preview-telemetry runs inside the direct_torque "
            "loop; do not combine it with --smoke-test or --ee-state-readout.",
            file=sys.stderr,
        )
        return 2
    ...
    torque_loop_logging = args.log_torque_loop_state or args.cartesian_preview_telemetry
    telemetry_enabled = args.ee_state_readout or torque_loop_logging
    telemetry_host = ""
    ...
    if args.smoke_test:
        ...
        urscript = build_urscript(
            ...
            log_torque_loop_state=torque_loop_logging,
            ...
            cartesian_preview_telemetry=args.cartesian_preview_telemetry,
    ...
    if args.smoke_test:
        ...
        print(f"Max torque per joint: {args.max_torque} Nm")
        print(f"Max torque rate per joint: {args.max_torque_rate} Nm/s")
        print(f"Max TCP force component: {args.max_tcp_force:.3f} N")
        print(f"Max TCP torque component: {args.max_tcp_torque:.3f} Nm")
        if args.cartesian_preview_telemetry:
            print("Cartesian preview telemetry: enabled, logged only, not commanded")
            print(f"Cartesian stiffness gains: {args.cartesian_stiffness}")
            print(f"Cartesian damping gains: {args.cartesian_damping}")
        if torque_loop_logging:
    ...
    if args.smoke_test:
        ...
        if args.cartesian_preview_telemetry:
            print(
                "Review cartesian_error and cartesian_wrench_cmd signs; the "
                "preview wrench was not mapped into joint torque."
            )
